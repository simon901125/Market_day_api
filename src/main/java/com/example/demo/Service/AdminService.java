...
public class AdminService implements AdminServiceInterface, EventStatusServiceInterface<Object> {
        ...
        // 設定管理員後台: 活動搜尋
        @Override
        public PageResponse<AdminEventListDto> getEventsList(AdminEventSearchDto request, int pageNumber,
                        int pageSize) {
                // ----------只撈頁面需要用到的欄位，避免撈出整張表----------
                ...
                for (Tuple row : rows) {
                        ...
                        String eventDate = String.format("%s - %s", startAt.format(dateFormatter),
        ...
        public AdminEventDetailDto getEventDetail(@NonNull Long eventId) throws IllegalArgumentException {
                ...
                String serviceHours = String.format(
                                ...
                                event.serviceStartTime() == null ? "營業開始時間"
                                                : event.serviceStartTime().format(timeFormatter),
                                event.serviceEndTime() == null ? "營業結束時間"
                                                : event.serviceEndTime().format(timeFormatter));
        ...
        public PageResponse<AdminUserListDto> getUserList(AdminUserSearchDto request, int pageNumber, int pageSize) {
                ...
                for (Tuple row : rows) {
                        ...
                        String userName = row.get("name", String.class) == null ? "使用者尚未填寫"
        ...
        public EventStatus changeToEventStatus(Object data) throws IllegalArgumentException {
                if (data instanceof MarketEvent) {
                        ...
                        return checkEventStatus(
                                        ...
                                        entity.getEndAt(),
                                        entity.getMaxBooths(),
                                        entity.getEventApplications() == null ? 0
